...
ERROR_t MCP2515_reset(void)
{
    ...
    MCP2515_setRegister(MCP_CANINTE, CANINTF_TX0IF | CANINTF_TX1IF | CANINTF_TX2IF |CANINTF_RX0IF | CANINTF_RX1IF | CANINTF_ERRIF | CANINTF_MERRF);
...
ERROR_t MCP2515_checkTXMailboxes(TXBn_t* txbn_ptr)
{
    if (!txbn_ptr)
    {
        return ERROR_FAIL;
